...
export default function FieldCardUser({
  ...
  onAddToCart?: (fieldId: string) => void
}) {
  ...
  return (
    ...
    >
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
          <div className="flex items-center space-x-1">
            {field.features && field.features.slice(0, 3).map((feature, index) => (
              <span key={index} className="bg-white/20 backdrop-blur-sm text-xs px-2 py-1 rounded-full">
                {feature}
              </span>
            ))}
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-1" style={{ fontFamily: playfairFont }}>{field.name}</h3>
        ...
        <div className="flex items-center mb-4">
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
              <svg 
                key={i}
                className={`h-4 w-4 ${i < Math.floor(field.rating) ? 'text-yellow-400' : 'text-gray-200'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
          ...
          <span className="ml-2 text-sm font-medium text-gray-600">{field.rating.toFixed(1)}</span>
        </div>
        <div className="flex items-center justify-between">
          <div>
            <span className="text-2xl font-bold text-green-600">{field.pricePerHour} VNĐ</span>
            <span className="text-sm font-normal text-gray-500">/hour</span>
          </div>
          <button 
            onClick={() => onAddToCart && onAddToCart(field.id)}
